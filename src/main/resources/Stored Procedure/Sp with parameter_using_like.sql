CREATE OR REPLACE PROCEDURE all_column_value_using_like(OUT result_text TEXT)
AS $$
DECLARE
...
BEGIN
  ...
  OPEN result_set FOR SELECT * FROM acc_chartofaccount where coaaccountname LIKE 'Ka%';
...
--CALL all_column_value_using_like('');
